// Planner.js — Weekly Planner with day selector
...
const PlannerSection = ({ plannerTasks, onAddTask }) => {
  ...
  const [selectedDay, setSelectedDay] = useState(() => {
    // Default to today's weekday index (Mon=0)
  ...
  const getDotColor = (priority) => {
    const colors = { HIGH: '#ff5f7e', MED: '#f5a623', LOW: '#3ddc84' };
    return colors[priority] || '#4f8eff';
  };
        ...
        {weekDates.map((date, i) => {
          ...
          return (
            ...
            >
              ...
              <span className="day-num" style={isToday && selectedDay !== i ? { color: 'var(--accent-teal)' } : {}}>
                {date.getDate()}
              </span>
          ...
          {[...dayTasks].sort((a,b) => (a.time||'').localeCompare(b.time||'')).map(task => (
            <div key={task.id} className="planner-task fade-in">
              ...
              <div className="planner-task-right">
                ...
                {task.duration && <span className="duration-badge">{task.duration} min</span>}
              </div>
            </div>
          ))}
        </div>
